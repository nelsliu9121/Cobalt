...
- Kept protocol v17 closed after merging the device-identity, picture-fit, and row-layout wire changes; no backward compatibility shim or fake picture handle was added.

## Finding fixes and RED/GREEN evidence

1. **Runtime/catalog compatibility**
   - RED: `bomtoon_feature_release_requires_current_runtime` failed with runtime `0.4.0` instead of `0.5.0`.
   - GREEN: `rtk cargo test --locked -p kobo-cli bomtoon_feature_release_requires_current_runtime` - 1 passed.
...
- `rtk cargo test -p kobo-ui -p kobo-protocol -p kobo-sdk -p kobo-bomtoon` - 892 passed, 4 ignored.
- `rtk cargo clippy -p kobo-ui -p kobo-protocol -p kobo-sdk -p kobo-bomtoon -p kobo-cli --all-targets --all-features -- -D warnings` - no issues.
- `rtk cargo build --workspace` - 0 errors and 117 pre-existing `kobod` dead-code warnings.
- `rtk cargo run --locked -p kobo-cli -- app-check --registry apps/catalog.json` - every registered Store app verified.
