...
impl FeaturedState {
    ...
    pub fn requeue_source(&mut self, generation: u64, source: FeatureSource) -> bool {
        let Some(batch) = self
            .batch
            .as_mut()
            .filter(|batch| batch.generation == generation)
        ...
        if batch.statuses.get(&source) != Some(&SourceStatus::Pending) {
            return false;
        }
        batch.statuses.insert(source, SourceStatus::Queued);
        batch.queued = FEATURE_SOURCES
            .into_iter()
            .filter(|source| batch.statuses.get(source) == Some(&SourceStatus::Queued))
            .collect();
        true
    }
